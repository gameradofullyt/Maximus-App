// ==========================================
// 1. SISTEMA DE ALERTAS PROFISSIONAIS
// ==========================================
const SystemModal = {
    init: () => {
    ...
    prompt: (title, message, callback) => {
        ...
        document.getElementById('sys-modal-btn-cancel').onclick = () => {
            modal.classList.remove('active');
        };
...
// ==========================================
// 2. BANCO DE DADOS E CONTA
// ==========================================
const BD = {

    init: () => {
        if(!localStorage.getItem('cine_users_db'))
            localStorage.setItem('cine_users_db', JSON.stringify([]));
    },

    getUsers: () =>
        JSON.parse(localStorage.getItem('cine_users_db')),

    saveUsers: (users) =>
        localStorage.setItem('cine_users_db', JSON.stringify(users)),

    getLogado: () =>
        JSON.parse(localStorage.getItem('cine_logged_in')),
    ...
    acaoModerador: (acao, targetUsername, amount = 0) => {
        ...
        if(targetUsername === '@gamerado' || targetUsername === '@gameradozinho'){
            if(acao !== 'pontos'){
                SystemModal.alert("Erro", "Ação negada: Você não pode punir moderadores supremos!");
                return;
            }
        }
        ...
        if(index === -1){
            SystemModal.alert("Erro", "Usuário não encontrado.");
            return;
        }

        if(acao === 'banir'){
            users[index].isBanned = true;
            SystemModal.alert("Sucesso", `Usuário ${targetUsername} foi banido!`);
        }

        if(acao === 'desbanir'){
            users[index].isBanned = false;
            SystemModal.alert("Sucesso", `Usuário ${targetUsername} foi desbanido.`);
        }

        if(acao === 'deletar'){
            users.splice(index, 1);
            SystemModal.alert("Sucesso", `Conta ${targetUsername} deletada permanentemente.`);
        }

        if(acao === 'pontos'){
            users[index].points =
                amount.toLowerCase() === "infinito"
                ? "INFINITO"
                : Number(users[index].points) + Number(amount);
    ...
    // ALTERAÇÃO DO TEMA (AGORA DARK E LIGHT)
    mudarTema: (tema) => {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            if(termo.length > 0){
                ...
                if(searchResultsScreen)
                    searchResultsScreen.classList.add('active');

                if(termoDigitado)
                    termoDigitado.innerText = termo;
                ...
                if(!encontrouAlgo){
                    ...
                    searchGridContainer.innerHTML =
                    '<p style="color:#888;text-align:center;grid-column:1/-1;padding-top:20px;">Nenhum conteúdo encontrado.</p>';
                }
                ...
                if(searchResultsScreen)
                    searchResultsScreen.classList.remove('active');
            }
        });
    }

    // APLICA O TEMA SALVO
    const temaSalvo = localStorage.getItem('cine_db_tema') || 'dark';
